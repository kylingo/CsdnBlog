...
import com.free.csdn.view.materialmenu.MaterialMenuDrawable.IconState;
...
public class MaterialMenuView extends View implements MaterialMenu {
	...
	private void init(Context context, AttributeSet attributeSet) {
		...
		try {
			int color = attr.getColor(R.styleable.MaterialMenuView_mm_color,
					MaterialMenuDrawable.DEFAULT_COLOR);
			int scale = attr.getInteger(R.styleable.MaterialMenuView_mm_scale,
					MaterialMenuDrawable.DEFAULT_SCALE);
			int transformDuration = attr.getInteger(
					R.styleable.MaterialMenuView_mm_transformDuration,
					MaterialMenuDrawable.DEFAULT_TRANSFORM_DURATION);
			int pressedDuration = attr.getInteger(R.styleable.MaterialMenuView_mm_pressedDuration,
					MaterialMenuDrawable.DEFAULT_PRESSED_DURATION);
			Stroke stroke = Stroke.valueOf(attr.getInteger(
					R.styleable.MaterialMenuView_mm_strokeWidth, 0));
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.HONEYCOMB) {
			widthMeasureSpec = MeasureSpec.makeMeasureSpec(drawable.getIntrinsicWidth() + paddingX,
					MeasureSpec.EXACTLY);
			heightMeasureSpec = MeasureSpec.makeMeasureSpec(drawable.getIntrinsicHeight()
					+ paddingY, MeasureSpec.EXACTLY);
			setMeasuredDimension(widthMeasureSpec, heightMeasureSpec);
		} else {
			setMeasuredDimension(drawable.getIntrinsicWidth() + paddingX,
					drawable.getIntrinsicHeight() + paddingY);
		}
	}
	...
	private void adjustDrawablePadding() {
		if (drawable != null) {
			drawable.setBounds(0, 0, drawable.getIntrinsicWidth() + getPaddingLeft()
					+ getPaddingRight(), drawable.getIntrinsicHeight() + getPaddingTop()
					+ getPaddingBottom());
		}
	}
